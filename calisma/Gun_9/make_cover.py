...
def create_shorts_thumbnail(lang='tr'):
    work_dir = os.path.join('calisma', 'Gun_9')
    raw_cover_path = os.path.join(work_dir, 'candidate_cover_haidt.jpg')
    if not os.path.exists(raw_cover_path):
        raw_cover_path = os.path.join(work_dir, 'candidate_cover_stress.jpg')
    is_en = (lang == 'en')
    output_cover_path = os.path.join(work_dir, 'Gun_9_Shorts_Kapak_EN.jpg' if is_en else 'Gun_9_Shorts_Kapak.jpg')
    ...
    if is_en:
        ...
        line1 = 'THE ATTENTION COLLAPSE'
        line2 = 'HOW PHONES BROKE YOUR BRAIN'
    else:
        badge_text = 'PODCAST ÖZEL'
        line1 = 'BÜYÜK DİKKAT ÇÖKÜŞÜ'
        line2 = 'TELEFON BEYNİNİ NASIL YIKTI?'

    max_text_width = 960
    font_hook = get_fitted_font(draw, line1, font_bold, 58, max_text_width)
    font_huge = get_fitted_font(draw, line2, font_bold, 64, max_text_width)
    ...
    print(f'[OK] Generated {lang.upper()} cover -> {output_cover_path} ({bg_rgb.size[0]}x{bg_rgb.size[1]})')
